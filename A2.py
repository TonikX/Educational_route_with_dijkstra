...
averagetaskteam={'0':0,'1':0.5,'11':1,'2':0,'3':0.9,'4':0} #Среднее арифметическое, отражающее степень охвата заданий из пройденных тем.
teampriority={'0':.3,'1':.6,'11':1,'2':.8,'3':.6,'4':.6} #Приоритетность темы




def edumap(graph,start,goal, previousTaskResult, minMarkWeight,maxMarkWeight,mark,completedtaskas,teampriority):
    shortest_distance={}
    ...
    if previousTaskResult < 0.6:
        ...
        thisTeamAgain = 0
        subTeam = 0
    print('параметры выполнения снова и саб темы',thisTeamAgain, subTeam)
    ...
    while unseenNodes: #пока есть непосещенные ноды в графе
        ...
        for childNode, weight in graph[minNode].items(): # weight - путь, childNode - ноды
            ...
            if  (minMarkWeight<mark[childNode]/30<maxMarkWeight) and not childNode in completedtaskas:
                ...
                print('sddf',averagetaskteam[str(teamMarkers[minNode])], teampriority[str(teamMarkers[minNode])])
                herf = (1.98)**((teampriority[str(teamMarkers[minNode])]-averagetaskteam[str(teamMarkers[minNode])])**4)-1
                print('приоритет темы текущей ноды',herf)
                if (weight + shortest_distance[minNode] - herf< shortest_distance[childNode]) or (thisTeamAgain == 1 and teamMarkers[minNode] == teamMarkers[completedtaskas[-1]] and childNode != start) or (subTeam == 1 and str(teamMarkers[minNode]) == str(teamMarkers[completedtaskas[-1]])+'1' and childNode != start) :
...
edumap(graph, 'b','l',0.4,0.6,1,mark,completedtaskas,teampriority)